...
def init_mov():
    ...
    while (True):
        if (diameter >= 125): break
        LCD.ellipse(Coordinate_x, Coordinate_y,diameter,diameter,LCD.emerald,True) # drow circle
        LCD.show()
        diameter += 2
        time.sleep(0.001)
    diameter = 1
    ...
    while (True):
        if (diameter >= 120): break
        LCD.ellipse(Coordinate_x, Coordinate_y,diameter,diameter,LCD.black,True) # drow circle
        LCD.show()
        diameter += 3
        time.sleep(0.001)

    LCD
    LCD.text("Setup Done", 80, 102, LCD.emerald)
    LCD.show()
    time.sleep(2)


# x軸方向の加速度を計測
def speed_meter(type):
    qmi8658=QMI8658()
    Touch.Mode = 0
    ...
    if (type <= 2 and type == 0):
        shaft = "X"
    elif (type <= 2 and type == 1):
        shaft = "Y"
    elif (type <= 2 and type == 2):
        shaft = "Z"
    ...
    while (True):
        ...
        LCD.fill(LCD.black)
        LCD.ellipse(119, 119, 119, 119,LCD.emerald,False)
        LCD.text(f"{shaft}_AcceleroMeter",60, 35,LCD.emerald)

        if (xyz[type] > 0):
            # LCD.write_text('LEFT',70,110,3,LCD.emerald)
            LCD.write_text(f"{xyz[type]:+.2f}", 20, 112, 5, LCD.emerald)
        else:
            LCD.write_text(f"{xyz[type]:+.2f}", 20, 112, 5, LCD.red)
    ...
    LCD.fill(LCD.black)
    LCD.write_text("Mode Change", 36, 112, 2, LCD.emerald)
    LCD.show()
    time.sleep(2)
